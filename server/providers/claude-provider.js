...
/**
 * Helper function to simulate streaming by yielding text in small chunks
 * @param {string} text - The full text to stream
 * @param {number} chunkSize - Number of characters per chunk (default: 3-5 chars for natural feel)
 */
async function* simulateStreaming(text, chunkSize = 4) {
  let i = 0;
  while (i < text.length) {
    // Variable chunk size for more natural feel (2-6 chars)
    const size = Math.floor(Math.random() * 5) + 2;
    const chunk = text.slice(i, i + size);
    yield chunk;
    i += size;
    // Small delay between chunks (5-15ms for smooth streaming)
        ...
        if (Array.isArray(content)) {
          for (const block of content) {
            if (block.type === 'text' && block.text) {
              ...
              for await (const textChunk of simulateStreaming(block.text)) {
                yield {
                  type: 'text',
                  content: textChunk,
                  provider: this.name
